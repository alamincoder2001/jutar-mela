		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
				<table class="record-table">
					...
					<tbody>
						<tr v-for="(saleReturn,index) in returns">
							<td>{{ index + 1 }}</td>
							<td>{{ saleReturn.ReturnDate }}</td>
							<td>{{ saleReturn.Customer_Name }}</td>
							<td>{{ saleReturn.Description }}</td>
							<td>{{ saleReturn.Product_Name }}</td>
							<td>{{ saleReturn.ProductQty }}</td>
							<td>{{ saleReturn.ReturnRate }}</td>
							<td style="text-align:right;">{{ saleReturn.ReturnAmount }}</td>
							<td style="text-align:center;">
								...
								<?php if ($this->session->userdata('accountType') != 'u') { ?>
									...
									<a href="" title="Delete Return" @click.prevent="deleteReturn(saleReturn.ProductReturn_SlNo)"><i class="fa fa-trash"></i></a>
								<?php } ?>
							</td>
						</tr>
						<tr style="font-weight:bold;">
							<td colspan="5" style="text-align:right;">Total</td>
							<td style="text-align:right;">{{ returns.reduce((prev, curr) => { return prev + parseFloat(curr.ProductQty)}, 0) }}</td>
							<td></td>
							<td style="text-align:right;">{{ returns.reduce((prev, curr) => { return prev + parseFloat(curr.ReturnAmount)}, 0) }}</td>
							<td></td>
						</tr>
					</tbody>
				</table>
