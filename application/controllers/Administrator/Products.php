...
class Products extends CI_Controller
{
    ...
    public function getProducts()
    {
        ...
        $products = $this->db->query("
            ...
            left join tbl_product_company pcom on p.Product_Company = pcom.company_id
            where p.status = 'a'
            $clauses
    ...
    public function getCurrentStock()
    {
        ...
        if (isset($data->companyId) && $data->companyId != '') {
            $clauses .= " and Product_Company = '$data->companyId'";
        }
    ...
    public function getTotalStock()
    {
        ...
        $stock = $this->db->query("
            select
                ...
                (select ((p.Product_SellingPrice - p.Product_SaleDiscount) * current_quantity)) as sale_stock_value
            from tbl_product p
            left join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
    ...
    public function barcodeGenerate($Product_SlNo)
    {
        ...
        $this->load->view('Administrator/products/barcode/auto_barcode', $data);
    }
...
<?php
    ...
    public function getProductSizes()
    {
        ...
        $result = $this->db->query("SELECT
            ...
            WHERE pd.Status = 'a'
            $clauses
            group by pd.product_size 
            order by  pd.product_size asc
        ")->result();


        foreach ($result as $key => $size) {
            $size->stock = $this->db->query("SELECT

                (select ifnull(sum(pd.PurchaseDetails_TotalQuantity), 0) 
                    from tbl_purchasedetails pd 
                    join tbl_purchasemaster pm on pm.PurchaseMaster_SlNo = pd.PurchaseMaster_IDNo
                    where pd.Product_IDNo = '$size->Product_SlNo'
                    and pd.product_size = '$size->product_size'
                    and pd.PurchaseDetails_branchID = '$this->brunch'
                    and pd.Status = 'a'
                    " . (isset($data->date) && $data->date != null ? " and pm.PurchaseMaster_OrderDate <= '$data->date'" : "") . "
                ) as purchased_quantity,
                ...
                (select ifnull(sum(prd.PurchaseReturnDetails_ReturnQuantity), 0) 
                    from tbl_purchasereturndetails prd 
                    join tbl_purchasereturn pr on pr.PurchaseReturn_SlNo = prd.PurchaseReturn_SlNo
                    where prd.PurchaseReturnDetailsProduct_SlNo = '$size->Product_SlNo'
                    and prd.product_size = '$size->product_size'
                    and prd.Status = 'a'
                    and prd.PurchaseReturnDetails_brachid = '$this->brunch'
                    " . (isset($data->date) && $data->date != null ? " and pr.PurchaseReturn_ReturnDate <= '$data->date'" : "") . "
                ) as purchase_returned_quantity,
                ...
                (select ifnull(sum(sd.SaleDetails_TotalQuantity), 0) 
                    from tbl_saledetails sd
                    join tbl_salesmaster sm on sm.SaleMaster_SlNo = sd.SaleMaster_IDNo
                    where sd.Product_IDNo = '$size->Product_SlNo'                    
                    and sd.product_size = '$size->product_size'
                    and sd.SaleDetails_BranchId = '$this->brunch'
                    and sd.Status = 'a'
                    " . (isset($data->date) && $data->date != null ? " and sm.SaleMaster_SaleDate <= '$data->date'" : "") . "
                ) as sold_quantity,

                (select ifnull(sum(pr.ProductQty), 0)
                    from tbl_productreturn pr 
                    where pr.product_id = '$size->Product_SlNo' 
                    and pr.product_size = '$size->product_size'
                    and pr.Status = 'a'
                    and pr.brunchId = '$this->brunch'
                    " . (isset($data->date) && $data->date != null ? " and pr.ReturnDate <= '$data->date'" : "") . "
                ) as sales_returned_quantity,

                (select ifnull(sum(dmd.DamageDetails_DamageQuantity), 0) 
                    from tbl_damagedetails dmd
                    join tbl_damage dm on dm.Damage_SlNo = dmd.Damage_SlNo
                    where dmd.Product_SlNo = '$size->Product_SlNo' 
                    and dmd.product_size = '$size->product_size'
                    and dmd.status = 'a'
                    and dm.Damage_brunchid = '$this->brunch'
                    " . (isset($data->date) && $data->date != null ? " and dm.Damage_Date <= '$data->date'" : "") . "
                ) as damaged_quantity,
    ...
    public function getSizewiseStock()
    {
        ...
        if (isset($data->productSize) && $data->productSize != '') {
            $clauses .= " and pdd.product_size = '$data->productSize'";
        }
        if (isset($data->companyId) && $data->companyId != '') {
            $clauses .= " and p.Product_Company = '$data->companyId'";
        }
        ...
        $result  = $this->db->query("SELECT
            ...
            (SELECT ifnull(sum(pd.PurchaseDetails_TotalQuantity), 0) 
                    from tbl_purchasedetails pd 
                    join tbl_purchasemaster pm on pm.PurchaseMaster_SlNo = pd.PurchaseMaster_IDNo
                    where pd.Product_IDNo = pdd.Product_IDNo
                    and pd.product_size = pdd.product_size
                    and pd.PurchaseDetails_branchID = '$this->brunch'
                    and pd.Status = 'a'
            ) as purchased_quantity,
            ...
            (SELECT ifnull(sum(prd.PurchaseReturnDetails_ReturnQuantity), 0) 
                from tbl_purchasereturndetails prd 
                join tbl_purchasereturn pr on pr.PurchaseReturn_SlNo = prd.PurchaseReturn_SlNo
                where prd.PurchaseReturnDetailsProduct_SlNo = pdd.Product_IDNo
                and prd.product_size = pdd.product_size
                and prd.Status = 'a'
                and prd.PurchaseReturnDetails_brachid = '$this->brunch'
            ) as purchase_returned_quantity,
            ...
            (SELECT ifnull(sum(sd.SaleDetails_TotalQuantity), 0) 
                from tbl_saledetails sd
                join tbl_salesmaster sm on sm.SaleMaster_SlNo = sd.SaleMaster_IDNo
                where sd.Product_IDNo = pdd.Product_IDNo                   
                and sd.product_size = pdd.product_size
                and sd.SaleDetails_BranchId = '$this->brunch'
                and sd.Status = 'a'
            ) as sold_quantity,
            ...
            (SELECT ifnull(sum(pr.ProductQty), 0)
                from tbl_productreturn pr 
                where pr.product_id = pdd.Product_IDNo
                and pr.product_size = pdd.product_size
                and pr.Status = 'a'
                and pr.brunchId = '$this->brunch'               
            ) as sales_returned_quantity,
            ...
            (SELECT ifnull(sum(dmd.DamageDetails_DamageQuantity), 0) 
                from tbl_damagedetails dmd
                join tbl_damage dm on dm.Damage_SlNo = dmd.Damage_SlNo
                where dmd.Product_SlNo = pdd.Product_IDNo
                and dmd.product_size = pdd.product_size
                and dmd.status = 'a'
                and dm.Damage_brunchid = '$this->brunch'
            ) as damaged_quantity,
            ...
            FROM tbl_purchasedetails pdd
            LEFT JOIN tbl_product p on p.Product_SlNo = pdd.Product_IDNo
            WHERE pdd.Status = 'a'
            $clauses
            and pdd.PurchaseDetails_branchID = '$this->brunch'
            group by p.Product_SlNo,pdd.product_size
        ")->result();
