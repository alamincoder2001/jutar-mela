...
class Sales extends CI_Controller
{
    ...
    public function addSales()
    {
        ...
        try {
            ...
            $sales = array(
                ...
                'SaleMaster_PaidAmount'          => $data->sales->return > 0 ? $data->sales->paid - $data->sales->return : $data->sales->paid,
                'SaleMaster_ReturnAmount'        => $data->sales->return,
                'SaleMaster_DueAmount'           => $data->sales->due,
                'SaleMaster_Previous_Due'        => $data->sales->previousDue,
                'SaleMaster_Description'         => $data->sales->note,
                'point'                          => $data->sales->point,
                'pointTaka'                      => $data->sales->pointTaka,
                'Status'                         => 'a',
            ...
            foreach ($data->cart as $cartProduct) {
                $saleDetails = array(
                    ...
                    'Discount_amount'           => $cartProduct->discountTotal,
                    'Commission_amount'         => $cartProduct->Commission_amount,
                    'SaleDetails_TotalAmount'   => $cartProduct->total,
                    'productPoint'              => $cartProduct->productPoint,
                    'Status'                    => 'a',
                    'AddBy'                     => $this->session->userdata("FullName"),
                    'AddTime'                   => date('Y-m-d H:i:s'),
    ...
    public function getSaleDetails()
    {
        ...
        if (isset($data->companyId) && $data->companyId != '') {
            $clauses .= " and p.Product_Company = '$data->companyId'";
        }
    ...
    public function getSalesRecord()
    {
        ...
        $returnClauses = "";
        if (isset($data->customerId) && $data->customerId != '') {
            $returnClauses .= " and pr.customer_id = '$data->customerId'";
        }
        if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
            $returnClauses .= " and pr.ReturnDate BETWEEN '$data->dateFrom' and '$data->dateTo' ";
        }

        $res['sale_return'] = $this->db->query("SELECT
            pr.*,
            c.Customer_Code,c.Customer_Name,c.Customer_Mobile,c.Customer_Address,
            p.Product_Code,p.Product_Name
            FROM tbl_productreturn pr
            LEFT JOIN tbl_customer c on c.Customer_SlNo = pr.customer_id
            LEFT JOIN tbl_product p on p.Product_SlNo = pr.product_id
            WHERE pr.Status = 'a'
            and pr.brunchId = ?
            $returnClauses
        ", $this->sbrunch)->result();
...
<?php
    ...
    function productReturn()
    {
        ...
        $data['content'] = $this->load->view('Administrator/sales/productReturn', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function addProductReturn()
    {
        ...
        try {
            ...
            $inputData['Status']   = 'a';
            $inputData['AddBy']    = $this->session->userdata('FullName');;
            $inputData['AddTime']  = date('Y-m-d H:i:s');
            $inputData['brunchId'] = $this->sbrunch;

            $this->db->insert('tbl_productreturn', $inputData);
            ...
            $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$data->ProductQty, $data->product_id, $this->session->userdata('BRANCHid')]);

            $res = ['success' => true, 'message' => 'Product return added successfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
        echo json_encode($res);
    ...
    public function updateProductReturn()
    {
        $data = json_decode($this->input->raw_input_stream);
        $PR_SlNo = $data->ProductReturn_SlNo;

        try {
            $oldData = $this->db->query("SELECT * FROM `tbl_productreturn` WHERE `ProductReturn_SlNo` = ?", $data->ProductReturn_SlNo)->row();
            $this->db->query("
                    update tbl_currentinventory 
                    set sales_return_quantity = sales_return_quantity - ? 
                    where product_id = ?
                    and branch_id = ?
                ", [$oldData->ProductQty, $oldData->product_id, $this->session->userdata('BRANCHid')]);


            $inputData             = (array)$data;
            unset($inputData['ProductReturn_SlNo']);
            $inputData['UpdateBy']    = $this->session->userdata('FullName');;
            $inputData['UpdateTime']  = date('Y-m-d H:i:s');
            $inputData['brunchId'] = $this->sbrunch;

            $this->db->where('ProductReturn_SlNo', $PR_SlNo)->update('tbl_productreturn', $inputData);
        ...
        ", [$data->ProductQty, $data->product_id, $this->session->userdata('BRANCHid')]);
            ...
            $res = ['success' => true, 'message' => 'Product return updated successfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
        echo json_encode($res);
    ...
    public function getProductReturn()
    {
        ...
        $clauses = "";
        if (isset($data->customerId) && $data->customerId != '') {
            $clauses .= " and pr.customer_id = '$data->customerId' ";
        }
        if ((isset($data->fromDate) && $data->fromDate != '') && (isset($data->toDate) && $data->toDate != '')) {
            $clauses .= " and pr.ReturnDate between '$data->fromDate' and '$data->toDate'";
        }


        $result = $this->db->query(
            "SELECT pr.*,p.Product_Article,c.Customer_Name,p.Product_Name,
             concat_ws(' - ', c.Customer_Code, c.Customer_Name, c.owner_name, c.Customer_Mobile) as display_name,
             concat(p.Product_Code,' - ',p.Product_Name,' - ',p.Product_Article) as display_text
            FROM tbl_productreturn pr
            LEFT JOIN tbl_customer c on c.Customer_SlNo = pr.customer_id
            left JOIN tbl_product p on p.Product_SlNo = pr.product_id
            WHERE pr.Status = 'a'
    ...
    public function deleteProductReturn()
    {
        $data = json_decode($this->input->raw_input_stream);

        try {
            $oldData = $this->db->query("SELECT * FROM `tbl_productreturn` WHERE `ProductReturn_SlNo` = ?", $data->prId)->row();
            $this->db->query("
                    update tbl_currentinventory 
                    set sales_return_quantity = sales_return_quantity - ? 
                    where product_id = ?
            ...
            $this->db->query("UPDATE tbl_productreturn SET Status='d' WHERE ProductReturn_SlNo = ?", $data->prId);


            $res = ['success' => true, 'message' => 'Product return deleted successfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
        echo json_encode($res);
    ...
    public function productReturnList()
    {
        ...
        $data['title'] = "Product Return List";
        $data['content'] = $this->load->view('Administrator/sales/product_return_list', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
