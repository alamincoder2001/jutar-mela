...
class Page extends CI_Controller
{
    ...
    {
        $sizes = $this->db->query("select * from tbl_sizes where status = 'a'")->result();
    ...
    public function saveSize()
    {
        ...
        try {
            ...
            unset($size['size_sl']);
            $size['status']    = 'a';
            $size['AddBy']     = $this->session->userdata("FullName");
            $size['AddTime']   = date("Y-m-d H:i:s");
            $size['branch_id'] = $this->session->userdata("BRANCHid");

            $this->db->insert('tbl_sizes', $size);

            $res = ['success' => true, 'message' => 'Size added Successfully'];
        } catch (Exception $ex) {
    ...
    public function updateSize()
    {
        ...
        try {
            $size = (array)$data;
            unset($size['size_sl']);
            $size['UpdateBy']     = $this->session->userdata("FullName");
            $size['UpdateTime']   = date("Y-m-d H:i:s");
            $size['branch_id'] = $this->session->userdata("BRANCHid");

            $this->db->where('size_sl', $sizeId)->update('tbl_sizes', $size);

            $res = ['success' => true, 'message' => 'Size Update Successfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
        echo json_encode($res);
    ...
    public function deleteSize()
    {
        ...
        $this->db->where('size_sl', $data->size_sl)->set('status', 'd')->update('tbl_sizes');
    ...
    {
        $companies = $this->db->query("select * from tbl_product_company where status = 'a'")->result();
    ...
    public function add_Company()
    {
        ...
        $data['content'] = $this->load->view('Administrator/add_Company', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function saveCompany()
    {
        ...
        try {
            ...
            $company = (array)$data;
            $company['status'] = 'a';
            $company['AddBy'] = $this->session->userdata("FullName");
            $company['AddTime'] = date('Y-m-d H:i:s');
            $company['branch_id'] = $this->session->userdata("BRANCHid");

            $this->db->insert('tbl_product_company', $company);
            $res = ['success' => true, 'message' => 'Company added successfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function updateCompany()
    {
        ...
        try {
            ...
            $company['UpdateBy'] = $this->session->userdata("FullName");
            $company['UpdateTime'] = date('Y-m-d H:i:s');

            $this->db->where('company_id', $data->company_id)->update('tbl_product_company', $company);
            $res = ['success' => true, 'message' => 'Company updated successfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    {
        $data = json_decode($this->input->raw_input_stream);
        $this->db->where('company_id', $data->companyId)->set('status', 'a')->update('tbl_product_company');
    }
    ...
    public function company_profile_Update()
    {
        ...
        $data['Company_Name']  = $this->input->post('Company_name', true);
        $data['Repot_Heading'] = $this->input->post('Description', true);
        $data['invoice_note']  = $this->input->post('invoice_note', true);
