...
class Model_Table extends CI_Model
{
    ...
    public function generateSalesInvoice()
    {
        ...
        $invoice = date('y') . $branchNo . "000001";
        $year = date('y');
        ...
        if ($sales->num_rows() != 0) {
            ...
            $invoice = date('ymd') . $branchNo . (strlen($newSalesId) > count($zeros) ? $newSalesId : $zeros[count($zeros) - strlen($newSalesId)] . $newSalesId);
        }
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            (
                select ifnull(sum(pr.ReturnAmount), 0) from tbl_productreturn pr
                where pr.brunchId = " . $this->session->userdata('BRANCHid') . "
                and pr.Status = 'a'
                " . ($date == null ? "" : " and pr.ReturnDate < '$date'") . "
            ) as product_return,
            ...
            (
                select paid_purchase + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment + loan_payment + invest_payment + buy_asset + product_return
            ) as total_out,
            (
        ...
        $stock = $this->db->query("
            select * from(
                select
                    ...
                    p.Product_after_discount,
                    (select (p.Product_Purchase_Rate * current_quantity)) as stock_value,
                    (select (p.Product_after_discount * current_quantity)) as sale_stock_value,
                    pc.ProductCategory_Name,
                    b.brand_name,
                    u.Unit_Name
                from tbl_currentinventory ci
                left join tbl_product p on p.Product_SlNo = ci.product_id
